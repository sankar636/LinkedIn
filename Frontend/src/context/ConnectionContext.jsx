...
const ConnectionProvider = ({ children }) => {
    ...
    const fetchConnections = useCallback(async () => {
        ...
        try {
            ...
            const { data } = await axios.get(`${serverUrl}/connections/getConnections`, {
            ...
            (data.data || []).forEach(conn => {
                ...
                const otherUser = conn.requester._id === userId ? conn.recipient : conn.requester;
                ...
                if (conn.status === "accepted") {
                    ...
                    } else { // Otherwise, it's an outgoing request
    ...
    const sendConnectionRequest = useCallback(async (recipientId) => {
        const token = getToken();
        try {
            await axios.post(`${serverUrl}/connections/connect/${recipientId}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const acceptConnection = useCallback(async (requesterId) => {
        const token = getToken();
        try {
            await axios.post(`${serverUrl}/connections/accept/${requesterId}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            
            const requestToAccept = connectionRequests.find(req => req.id === requesterId);
            if (requestToAccept) {
                setConnectionRequests(prev => prev.filter(req => req.id !== requesterId));
    ...
    const ignoreConnection = useCallback(async (otherUserId) => {
        const token = getToken();
        try {
            await axios.post(`${serverUrl}/connections/ignore/${otherUserId}`,{}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const requestToIgnore = connectionRequests.find(req => req.id === otherUserId)
            if(requestToIgnore){
            setConnectionRequests(prev => prev.filter(req => req.id !== otherUserId));
    ...
    const connectionValue = useMemo(() => ({
        ...
        loading, error, connections, connectionRequests, outgoingRequests,ignoreRequests,
        fetchConnections, sendConnectionRequest, acceptConnection, ignoreConnection
    ]);
