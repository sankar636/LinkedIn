...
const ConnectionView = () => {
    ...
    const handleIgnore = useCallback(async (requesterId) => {
        try {
    ...
    // Group connections
    const invitations = connectionRequests.filter(req => !ignoreRequests.includes(req.id));
    const ignored = connectionRequests.filter(req => ignoreRequests.includes(req.id));
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4">
            <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex border-b border-gray-200 mb-6 md:w-full overflow-x-auto md:overflow-x-hidden scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
                    <button
                        className={`py-2 px-4 font-medium text-sm flex items-center min-w-[50%] md:min-w-fit ${activeTab === "Invitations"
                    ...
                    <button
                        className={`py-2 px-4 font-medium text-sm flex items-center min-w-[50%] md:min-w-fit ${activeTab === "Connections"
                                ? "text-blue-600 border-b-2 border-blue-600"
                                : "text-gray-600"
                            }`}
                    ...
                    <button
                        className={`py-2 px-4 font-medium text-sm flex items-center min-w-[50%] md:min-w-fit ${activeTab === "Pending"
                                ? "text-blue-600 border-b-2 border-blue-600"
                                : "text-gray-600"
                            }`}
                    ...
                    <button
                        className={`py-2 px-4 font-medium text-sm flex items-center min-w-[50%] md:min-w-fit ${activeTab === "Ignored"
                                ? "text-blue-600 border-b-2 border-blue-600"
                                : "text-gray-600"
                            }`}
                ...
                {activeTab === 'Connections' && (
                    <div role="tabpanel" aria-labelledby="Connections-tab">
                        <h2 className="text-lg font-semibold text-gray-700 mb-4">People you're Connected</h2>
                        {accepted.length > 0 ? (
                            accepted.map(conn => (
                                <ConnectionCard
                                    key={conn.id}
                    ...
                    <div role="tabpanel" aria-labelledby="Connections-tab">
                        ...
                        {pending.length > 0 ? (
                            pending.map(request => (
                                <ConnectionCard
                                    key={request._id}
                                    connection={{ id: request._id, name: `${request.firstname} ${request.lastname}`, title: request.username, profileImage: request.profileImage }}
                                    isRequest={false}
                                    connectionStatus="pending"
                                    onConnect={handleSendRequest}
                                />
