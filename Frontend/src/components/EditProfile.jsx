...
const EditProfile = () => {
  ...
  const [educationInput, setEducationInput] = useState({ college: "", degree: "", fieldOfStudy: "" });
  const [experienceInput, setExperienceInput] = useState({ title: "", company: "", description: "" });
  ...
  return (
    <div className="w-full h-[100vh] fixed top-0 left-0 z-[100] flex justify-center items-center">
      ...
      <div className="relative w-[90%] max-w-[600px] bg-white shadow-lg rounded-lg z-[200] p-6 overflow-y-auto h-[90vh]">
        <div
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex gap-2 mb-2">
              ...
              />
              <button type="button" onClick={addSkill} className="bg-blue-600 text-white px-4 py-2 rounded">Add</button>
            </div>
            <div className="flex flex-wrap gap-2">
              {formData.skills.map((skill, idx) => (
                <span key={idx} className="bg-gray-200 px-3 py-1 rounded-full flex items-center gap-2">
                  {skill}
                  <button type="button" onClick={() => removeSkill(skill)} className="text-red-500">×</button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex flex-col gap-2 mb-2">
              ...
              <input
                ...
                onChange={(e) => setEducationInput({ ...educationInput, fieldOfStudy: e.target.value })}
                className="border rounded px-3 py-2"
              />
              <button type="button" onClick={addEducation} className="bg-green-600 text-white px-4 py-2 rounded">Add Education</button>
            </div>
            {formData.education.map((edu, idx) => (
              <div key={idx} className="bg-gray-100 px-3 py-2 rounded mb-2 flex justify-between items-center">
                {edu.college} - {edu.degree}
                <button type="button" onClick={() => removeEducation(idx)} className="text-red-500">×</button>
              </div>
            ))}
          </div>
          ...
          <div className="border rounded p-3">
            ...
            <div className="flex flex-col gap-2 mb-2">
              ...
              <textarea
                ...
                className="border rounded px-3 py-2"
              />
              <button type="button" onClick={addExperience} className="bg-green-600 text-white px-4 py-2 rounded">Add Experience</button>
            </div>
            {formData.experience.map((exp, idx) => (
              <div key={idx} className="bg-gray-100 px-3 py-2 rounded mb-2 flex justify-between items-center">
                {exp.title} @ {exp.company}
                <button type="button" onClick={() => removeExperience(idx)} className="text-red-500">×</button>
              </div>
            ))}
          </div>
