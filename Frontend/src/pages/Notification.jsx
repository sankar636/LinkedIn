  ...
  switch (content) {
    case 'Connection_Request':
      return {
        ...
        message: (
          <span>
            <span className="font-bold">{senderName}</span> sent you a
            connection request.
          </span>
        ),
      };
    case 'Request_Accepted':
      return {
        ...
        message: (
          <span>
            <span className="font-bold">{senderName}</span> accepted your
            connection request.
          </span>
        ),
      };
    case 'Post_Like':
      ...
      return {
        ...
        message: (
          <div>
            <span>
              <span className="font-bold">{senderName}</span> liked your post.
            </span>
            {postSnippet && (
              <blockquote className="mt-2 border-l-4 border-gray-300 pl-3 text-sm text-gray-600 italic">
                {postSnippet}
              </blockquote>
            )}
          </div>
    ...
    case 'New_Comment':
      ...
      return {
        ...
        message: (
          <div>
            <span>
              <span className="font-bold">{senderName}</span> commented on your
              post.
            </span>
            {commentSnippet && (
              <blockquote className="mt-2 border-l-4 border-blue-400 pl-3 text-sm text-gray-800 italic">
...
const Notification = () => {
  ...
  const handleMarkAllRead = async () => {
    try {
      const token = getToken();
      await axios.post(`${serverUrl}/notifications/mark-read`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (
      notification.content === 'Post_Like' ||
      notification.content === 'New_Comment'
    ) {
      if (notification.sender && notification.sender.username) {
        // navigate to the sender profile page
        navigate(`/profile/${notification.sender.username}`);
      } else {
        toast.error('Post details not available.');
      }
    } else if (
      notification.content === 'Connection_Request' ||
      notification.content === 'Request_Accepted'
    ) {
      if (notification.sender && notification.sender.username) {
        navigate(`/profile/${notification.sender.username}`);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <Navbar />
      <main className="mx-auto mt-[70px] max-w-4xl px-4 py-8">
          ...
          <div className="border-b p-2">
            <div className="flex space-x-2">
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold ${filter === 'all' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-200'}`}
              >
                All
              </button>
              <button
                onClick={() => setFilter('unread')}
                className={`rounded-full px-4 py-2 text-sm font-semibold ${filter === 'unread' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-200'}`}
              >
                Unread
              </button>
            </div>
                ...
                {filteredNotifications?.map((notification) => {
                  ...
                  return (
                    <li
                      ...
                      className={`flex cursor-pointer items-start gap-4 p-4 transition-colors ${!notification.read ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
                    >
                      <div
                        className="z-10 mt-1 flex-shrink-0"
                        onClick={(e) =>
                          handleProfileRedirect(e, notification.sender)
                        }
                      >
                        {icon}
                      </div>
